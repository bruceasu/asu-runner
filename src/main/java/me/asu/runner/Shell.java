...
abstract public class Shell
{
    ...
    public static String[] getCheckProcessIsAliveCommand(String pid)
    {
        return Shell.WINDOWS ? new String[]{Shell.WINUTILS, "task", "isAlive", pid}
                : new String[]{"kill", "-0", IS_SETSID_AVAILABLE ? "-" + pid : pid};
    }
    ...
    public static String[] getSignalKillCommand(int code, String pid)
    {
        return Shell.WINDOWS ? new String[]{Shell.WINUTILS, "task", "kill", pid}
                : new String[]{"kill", "-" + code, IS_SETSID_AVAILABLE ? "-" + pid : pid};
    }
    ...
    public static String getQualifiedBinPath(String executable) throws IOException
    {
        ...
        String fullExeName = MY_HOME_DIR + File.separator + "bin" + File.separator + executable;

        File exeFile = new File(fullExeName);
        if (!exeFile.exists()) {
            throw new IOException(
                    "Could not locate executable " + fullExeName + " in the binaries.");
        }
